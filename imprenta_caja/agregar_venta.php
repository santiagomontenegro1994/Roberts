...
// --- PRECARGA DE PRODUCTOS Y VARIANTES PARA JS (0 AJAX) ---
$productos_memoria = [];
$sql_prod = "SELECT id, titulo, precio, stock, stock_infinito, imagen 
             FROM productos 
             WHERE idActivo = 1 
             ORDER BY titulo ASC";
...
if ($res_prod) {
    while ($p = mysqli_fetch_assoc($res_prod)) {
        // Cargar variantes si las tiene
        $variantes = [];
        $res_var = mysqli_query($MiConexion, "SELECT id, color_nombre, color_hex, stock, nombre_imagen FROM productos_imagenes WHERE id_producto = " . (int)$p['id']);
        if ($res_var) {
...
<style>
    .modal-insumos-body { max-height: 70vh; overflow-y: auto; }
        ...
        <div class="card">
            <div class="card-body">
                <form method="post" id="formVenta">
                    ...
                    <!-- Campo Oculto para enviar los insumos seleccionados -->
                    ...
                    <!-- Panel de Resumen de Insumos Seleccionados -->
  ...
  <div class="modal-dialog modal-lg modal-dialog-centered">
    <div class="modal-content border-0 shadow-lg">
      ...
      <div class="modal-body p-3">
        <!-- Buscador en tiempo real (Memoria JS) -->
        <div class="mb-3">
            <input type="text" id="inputBuscarInsumo" class="form-control form-control-lg border-primary shadow-sm" placeholder="🔍 Escribí para buscar insumo por nombre...">
        </div>
        ...
        <div class="row">
            ...
            <!-- Columna Derecha: Insumos Seleccionados (Mini Carrito) -->
      ...
      <div class="modal-footer bg-light">
        ...
        <button type="button" class="btn btn-success fw-bold" onclick="confirmarInsumosModal()"><i class="bi bi-check-circle me-1"></i> Confirmar y Calcular Monto</button>
      </div>
    </div>
  </div>
</div>
...
<script>
    // PRECARGA EN MEMORIA JS (0 CONSULTAS AL SERVIDOR AL BUSCAR)
    const CATALOGO_PRODUCTOS = <?php echo json_encode($productos_memoria); ?>;
    let CARRITO_INSUMOS = [];
    ...
    function abrirModalInsumos() {
        ...
        modal.show();
        document.getElementById('inputBuscarInsumo').focus();
        renderizarCatalogo('');
    }

    // Renderizar lista filtrada desde la memoria de JavaScript
    function renderizarCatalogo(filtro) {
        ...
        filtrados.forEach(p => {
            ...
            // Determinar estilo de badge de stock (Permite vender aun con stock <= 0)
            let stockBadge = '';
            ...
            if (p.variantes && p.variantes.length > 0) {
                selectorVariantesHTML = `<select class="form-select form-select-sm my-1 selector-variante-item" id="var_select_${p.id}">
                    <option value="0">Color Principal (${p.stock} u.)</option>`;
                p.variantes.forEach(v => {
                    selectorVariantesHTML += `<option value="${v.id}" data-nombre="${v.color_nombre}" data-stock="${v.stock}">${v.color_nombre} (${v.stock} u.)</option>`;
                });
    ...
    function agregarAlCarritoModal(idProd) {
        ...
        cantInput.value = 1; // Resetear input
        renderizarCarritoModal();
    }

    // Eliminar ítem del carrito modal
    ...
    // Confirmar modal y traspasar datos al formulario principal
    function confirmarInsumosModal() {
        ...
        // Guardar JSON para PHP
        inputHidden.value = JSON.stringify(CARRITO_INSUMOS);

        // Renderizar resumen visual debajo del tipo de entrada
        ...
        // Auto-calcular y colocar el dinero en el campo de la caja
        if (totalGeneral > 0) {
    ...
    // Detectar clic en el botón de Tipo de Entrada "Insumos"
    document.querySelectorAll('.tipo-movimiento').forEach(button => {
        button.addEventListener('click', function() {
            ...
            // Si el nombre contiene la palabra "insumo", abre el modal automáticamente
    ...
    // Lógica original de formato de dinero

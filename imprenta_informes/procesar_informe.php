...
function obtenerDatosMes($conexion, $m, $a) {
    ...
    // Reordenar por monto (para que se vea bien en la lista)
    usort($listaIngresos, function($a, $b) { return $b['monto'] <=> $a['monto']; });

    // Calcular porcentajes finales para la lista
    foreach($listaIngresos as &$row) {
        $row['porcentaje'] = ($totalIngresos > 0) ? number_format(($row['monto'] / $totalIngresos) * 100, 1) . '%' : '0%';
    }

...
// LÓGICA MTD (Month-To-Date): Si es el mes actual, obtenemos el día de hoy.
$es_mes_actual = ($anio == date('Y') && $mes == date('m'));
$dia_limite = $es_mes_actual ? (int)date('d') : null;

// --- FUNCIÓN PRINCIPAL ---

function obtenerDatosMes($conexion, $m, $a, $dia_limite = null) {
    // Filtros dinámicos de días para recortes MTD
    $filtroDiaCaja = $dia_limite ? " AND DAY(c.Fecha) <= $dia_limite" : "";
    $filtroDiaRetiro = $dia_limite ? " AND DAY(fecha) <= $dia_limite" : "";
    $filtroDiaRetiroAlias = $dia_limite ? " AND DAY(r.fecha) <= $dia_limite" : "";

    // 1. OBTENER DIFERENCIAS DE CAJA (Se usarán tanto en Ventas Totales como en Efectivo)
    $sqlDifPos = "SELECT SUM(dc.monto) as total FROM detalle_caja dc JOIN caja c ON dc.idCaja = c.idCaja WHERE MONTH(c.Fecha) = '$m' AND YEAR(c.Fecha) = '$a' AND dc.idTipoMovimiento = 15" . $filtroDiaCaja;
    $difPositiva = floatval(mysqli_fetch_assoc(mysqli_query($conexion, $sqlDifPos))['total']);

    $sqlDifNeg = "SELECT SUM(dc.monto) as total FROM detalle_caja dc JOIN caja c ON dc.idCaja = c.idCaja WHERE MONTH(c.Fecha) = '$m' AND YEAR(c.Fecha) = '$a' AND dc.idTipoMovimiento = 14" . $filtroDiaCaja;
    $difNegativa = floatval(mysqli_fetch_assoc(mysqli_query($conexion, $sqlDifNeg))['total']);

    // 2. OBTENER INGRESOS BRUTOS DE CAJA
    // *** CAMBIO: Agregamos JOIN tipo_pago y filtro idActivo = 1 ***
    $sqlEntradas = "SELECT SUM(dc.monto) as total FROM detalle_caja dc
                    ...
                    AND tp.idActivo = 1" . $filtroDiaCaja; // Excluye los inactivos
    ...
    // 3. OBTENER TOTALES DE RETIROS (Salidas Totales)
    // Sigue igual: Solo retiros, sin caja fuerte (9), sin dif neg (14)
    $sqlRetiros = "SELECT SUM(monto) as total FROM retiros 
                   WHERE MONTH(fecha) = '$m' AND YEAR(fecha) = '$a'
                   AND idTipoMovimiento NOT IN (9, 14, 15)" . $filtroDiaRetiro; 
    $montoRetiros = floatval(mysqli_fetch_assoc(mysqli_query($conexion, $sqlRetiros))['total']);

    // 4. CÁLCULO DE TOTALES FINALES
    // *** CAMBIO: Ventas Totales ahora INCLUYE las diferencias ***
    // Ventas = (Ventas Reales Activas) + (Sobra Plata) - (Falta Plata)
    ...
    // C) EFECTIVO
    // *** CAMBIO: Filtramos también aquí tp.idActivo = 1 ***
    $sqlEfecEnt = "SELECT SUM(dc.monto) as monto FROM detalle_caja dc 
                   ...
                   AND tp.idActivo = 1" . $filtroDiaCaja; // Excluye inactivos
    $montoEntEfec = floatval(mysqli_fetch_assoc(mysqli_query($conexion, $sqlEfecEnt))['monto']);
    
    // *** CAMBIO: El Efectivo TAMBIÉN incluye las diferencias para que la matemática cierre ***
...
$anterior = obtenerDatosMes($MiConexion, $mesAnt, $anioAnt, $dia_limite);

echo json_encode([
    'ok' => true,
    'datos' => $actual,
    'previo' => $anterior,
    'es_parcial' => $es_mes_actual,
    'dias_parciales' => $dia_limite
]);
?>